...
static QueueHandle_t queue_create_and_register(
...
///// \note MUST REMAIN UNCHANGED: \b osKernelRunning shall be consistent in
// every CMSIS-RTOS.
...
static TaskHandle_t cmsis2rtos_task(osThreadId v)
{
    union {
        TaskHandle_t h ;
        osThreadId v ;
...
static osThreadId rtos2cmsis_task(TaskHandle_t h)
{
    union {
        TaskHandle_t h ;
        osThreadId v ;
    ...
    void * argument)
{
    ...
    return rtos2cmsis_task(handle) ;
}

#if ( ( INCLUDE_xTaskGetCurrentTaskHandle == 1) || (configUSE_MUTEXES == 1 ) )
osThreadId osThreadGetId(void)
{
    return rtos2cmsis_task( xTaskGetCurrentTaskHandle() ) ;
}
...
{
    vTaskDelete( cmsis2rtos_task(thread_id) ) ;
...
///// \param[in] millisec          \ref CMSIS_RTOS_TimeOutValue or 0 in case of
// no time-out
    ...
    int32_t signal)
{
    ...
    if ( xPortInIsrContext() ) {
        ...
        if ( xTaskNotifyFromISR(cmsis2rtos_task(thread_id), (uint32_t) signal,
                                eSetBits,
    ...
    else if ( xTaskNotify(cmsis2rtos_task(thread_id), (uint32_t) signal,
                          eSetBits) != pdPASS ) {
        return osErrorOS ;
    }
    ...
    uint32_t millisec)
{
    ...
    if ( xPortInIsrContext() ) {
        ...
        if ( xTaskNotifyWait(
                 0, // ulBitsToClearOnEntry: non cancello niente quando mi metto
                    // in attesa
                 (uint32_t) signals,
...
static QueueHandle_t cmsis2rtos_mutex(osMutexId v)
{
    union {
        QueueHandle_t h ;
        osMutexId v ;
...
static osMutexId rtos2cmsis_mutex(QueueHandle_t h)
{
    union {
        QueueHandle_t h ;
        osMutexId v ;
...
osMutexId osMutexCreate(const osMutexDef_t * mutex_def)
{
    ...
    if ( xPortInIsrContext() ) {
        ...
        return rtos2cmsis_mutex( xSemaphoreCreateMutex() ) ;
    }
}
    ...
    uint32_t millisec)
{
    ...
    if ( xPortInIsrContext() ) {
        ...
        return pdTRUE ==
               xSemaphoreTake(cmsis2rtos_mutex(mutex_id), attesa) ?
...
osStatus osMutexRelease(osMutexId mutex_id)
{
    ...
    if ( xPortInIsrContext() ) {
        ...
        return pdTRUE ==
               xSemaphoreGive( cmsis2rtos_mutex(mutex_id) ) ?
...
osStatus osMutexDelete(osMutexId mutex_id)
{
    ...
    if ( xPortInIsrContext() ) {
        ...
        vSemaphoreDelete( cmsis2rtos_mutex(mutex_id) ) ;
...
///// \note MUST REMAIN UNCHANGED: \b osSemaphoreWait shall be consistent in
// every CMSIS-RTOS.
...
static QueueHandle_t cmsis2rtos_msgq(osMessageQId v)
{
    union {
        QueueHandle_t h ;
        osMessageQId v ;
...
static osMessageQId rtos2cmsis_msgq(QueueHandle_t h)
{
    union {
        QueueHandle_t h ;
        osMessageQId v ;
    ...
    osThreadId thread_id)
{
    ...
    if ( xPortInIsrContext() ) {
        ...
        return rtos2cmsis_msgq( queue_create_and_register(queue_def->queue_sz,
                                                          queue_def->item_sz,
                                                          queue_def->nome) ) ;
    }
}
...
{
    TickType_t ticks = ms_in_tick(millisec) ;
    QueueHandle_t queue_id = cmsis2rtos_msgq(mq) ;
...
{
    xQueueReset( cmsis2rtos_msgq(m) ) ;
}

int ose_MessageWaiting(osMessageQId mq)
{
    QueueHandle_t queue_id = cmsis2rtos_msgq(mq) ;
